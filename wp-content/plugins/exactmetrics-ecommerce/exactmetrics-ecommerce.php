...
/**
 * Plugin Name:       ExactMetrics - eCommerce Addon
 * Plugin URI:        https://www.exactmetrics.com
 * Description:       Adds eCommerce tracking options to ExactMetrics
 * Author:            ExactMetrics Team
 * Author URI:        https://www.exactmetrics.com
 * Version:           2.4.0
 * Requires at least: 4.8.0
 * Requires PHP:      5.5
 * Text Domain:       exactmetrics-ecommerce
 ...
 * @since 6.0.0
 *
 * @package ExactMetrics_eCommerce
 * @author  Chris Christoff
 */
class ExactMetrics_eCommerce
{
	/**
	 ...
	 * @var string
	 */
	public $plugin_name = 'ExactMetrics eCommerce';
	 ...
	 * @var string
	 */
	public $plugin_slug = 'exactmetrics-ecommerce';
	...
	private function check_compatibility()
	{
		...
		$compatibility = ExactMetrics_Ecommerce_Compatibility_Check::get_instance();
	...
	{
		if (!defined('EXACTMETRICS_VERSION') || !defined('EXACTMETRICS_PRO_VERSION') || version_compare(EXACTMETRICS_VERSION, '7.8.1', '<')) {
	...
	public function modify_autoupdater_setting_html($html, $plugin_file)
	{
		if (
			...
			$html = sprintf(
				'<a href="%s">%s</a>',
				'https://www.exactmetrics.com/docs/go-lite-pro/?utm_source=liteplugin&utm_medium=plugins-autoupdate&utm_campaign=upgrade-to-autoupdate&utm_content=exactmetrics-ecommerce',
				__('Enable the ExactMetrics PRO plugin to manage auto-updates', 'exactmetrics-ecommerce')
			);
		}
	...
	{
		require_once plugin_dir_path(__FILE__) . 'includes/class-exactmetrics-ecommerce-helper.php';
		...
		$tracking_code_v4  = function_exists('exactmetrics_get_v4_id') && exactmetrics_get_v4_id();
		$has_tracking      = !empty($tracking_code_v4);

		if ($has_tracking) {
            if (class_exists('Easy_Digital_Downloads') && apply_filters('exactmetrics_track_edd', true)) {
                ExactMetrics_eCommerce_EDD_Integration::get_instance();
            }

            if (class_exists('WooCommerce') && apply_filters('exactmetrics_track_woocommerce', true)) {
                ExactMetrics_eCommerce_WooCommerce_Integration::get_instance();
            ...
            if (defined('MEPR_VERSION') && version_compare(MEPR_VERSION, '1.3.43', '>') && apply_filters('exactmetrics_track_memberpress', true)) {
                ExactMetrics_eCommerce_MemberPress_Integration::get_instance();
            }

            if (function_exists('LLMS') && version_compare(LLMS()->version, '3.32.0', '>=') && apply_filters('exactmetrics_track_lifterlms', true)) {
                ExactMetrics_eCommerce_LifterLMS_Integration::get_instance();
            }

            if (class_exists('MemberMouse') && apply_filters('exactmetrics_track_mm', true)) {
                ExactMetrics_eCommerce_MemberMouse_Integration::get_instance();
            }
...
//			if (class_exists('Easy_Digital_Downloads') && apply_filters('exactmetrics_track_edd', true)) {
//				new ExactMetrics_GA_EDD_eCommerce_Tracking();
//			}
//
//			if (class_exists('WooCommerce') && apply_filters('exactmetrics_track_woocommerce', true)) {
//				new ExactMetrics_GA_Woo_eCommerce_Tracking();
//			}
//
//			if (defined('MEPR_VERSION') && version_compare(MEPR_VERSION, '1.3.43', '>') && apply_filters('exactmetrics_track_memberpress', true)) {
//				new ExactMetrics_GA_MemberPress_eCommerce_Tracking();
//			}
//
//			if (function_exists('LLMS') && version_compare(LLMS()->version, '3.32.0', '>=') && apply_filters('exactmetrics_track_lifterlms', true)) {
//				new ExactMetrics_GA_LifterLMS_eCommerce_Tracking();
//			}
		}
	}
...
?>
		<div class="error">
			<p><?php echo esc_html__('Please install ExactMetrics Pro version 7.8.1 or newer to use the ExactMetrics eCommerce addon.', 'exactmetrics-ecommerce'); ?></p>
		</div>
<?php
	}
	 ...
	 *
	 * @return object The ExactMetrics_eCommerce object.
	 * @since 6.0.0
	 *
	 */
	public static function get_instance()
	{
		if (!isset(self::$instance) && !(self::$instance instanceof ExactMetrics_eCommerce)) {
			self::$instance = new ExactMetrics_eCommerce();
		}
	...
	public function deactivate_plugins()
	{
		if (function_exists('ExactMetrics')) {
			if (ExactMetrics()->auth->get_v4_id()) {
...
$exactmetrics_ecommerce = ExactMetrics_eCommerce::get_instance();
