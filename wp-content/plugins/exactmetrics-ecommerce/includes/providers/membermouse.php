...
class ExactMetrics_eCommerce_MemberMouse_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	 ...
	 */
	const TABLE_NAME = "mi_mm_exactmetrics_meta";
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_MemberMouse_Integration();
	...
	public function store_user_id( $data ) {
		$ga_uuid = exactmetrics_get_client_id();
		if ( $ga_uuid && isset( $data["order_transaction_id"] ) && isset( $data["order_id"] ) ) {
			$cookie = exactmetrics_get_cookie();
			$this->store_transaction_meta( $data["order_transaction_id"], $data["order_id"], $ga_uuid, $cookie );
		}

		if ( ( $measurement_id = exactmetrics_get_v4_id_to_output() ) && isset( $data["order_transaction_id"] ) ) {
	...
	protected function save_user_session_id( $payment_id, $measurement_id ) {
		if ( function_exists( 'exactmetrics_get_browser_session_id' ) ) {
			$session_id = exactmetrics_get_browser_session_id( $measurement_id );
			$this->store_user_session_id( $payment_id, $session_id );
		}
	}

	private function track_checkout_v4( $product_id ) {
		if ( ! function_exists( 'exactmetrics_get_v4_id_to_output' ) ||
		     ! function_exists( 'exactmetrics_mp_collect_v4' ) ||
	...
	public function checkout_page() {
		...
		if ( exactmetrics_is_page_reload() ) {
			return;
		}

		// Don't track test mode.
		if ( ExactMetrics_eCommerce_Helper::mm_test_mode() ) {
			return;
		}
	...
	private function track_transaction_v4( $data, $cid, $discount, $is_rebill ) {
		...
		if ( exactmetrics_get_option( 'userid', false ) ) {
	...
	public function do_transaction( $data, $is_rebill = false ) {
		...
		$skip_renewals = apply_filters( 'exactmetrics_ecommerce_skip_renewals', true );
		if ( $skip_renewals && $is_rebill ) {
			return;
		}

		$is_in_ga = ( $meta['exactmetrics_is_in_ga'] === true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_do_transaction_skip_ga', false, $txn_id );
		if ( $is_in_ga || $skip_ga ) {
			return;
		}
		...
		// Update in GA
		$this->store_transaction_meta( $txn_id, $order_id, $cid, $meta['exactmetrics_cookie'], true );
	}
	...
	public function undo_transaction( $data ) {
		...
		$is_in_ga = $meta['exactmetrics_refund_is_in_ga'];

		$skip_ga = apply_filters( 'exactmetrics_ecommerce_undo_transaction_skip_ga', false, $txn_id );
		if ( $is_in_ga || $skip_ga ) {
			return;
		}
	...
	protected function sync_table() {
		...
		if ( ! $synced ) {
			...
			$plugin = ExactMetrics_eCommerce::get_instance();
			...
			if ( ( $version === false ) || ( $version != $plugin->version ) ) {
				...
				$sql = "CREATE TABLE {$table_name} (
					...
					exactmetrics_cookie TEXT DEFAULT NULL,
					exactmetrics_is_in_ga TINYINT DEFAULT 0,
					exactmetrics_refund_is_in_ga TINYINT DEFAULT 0,
	...
	public function get_transaction_meta( $transaction_id ) {
		...
		$row['exactmetrics_is_in_ga']        = ( $row['exactmetrics_is_in_ga'] == 1 );
		$row['exactmetrics_refund_is_in_ga'] = ( $row['exactmetrics_refund_is_in_ga'] == 1 );
	 ...
	 * @param string $cookie The ExactMetrics cookie
	 */
	public function store_transaction_meta( $transaction_id, $order_id, $ga_uuid, $cookie, $exactmetrics_is_in_ga = null ) {
		global $wpdb;

		$table = self::get_table_name();
		if ( $exactmetrics_is_in_ga == null ) {
			$exactmetrics_is_in_ga = boolval( $exactmetrics_is_in_ga ) ? 1 : 0;
			$sql                      = "INSERT INTO {$table} ( transaction_id, ga_uuid, order_id, exactmetrics_cookie ) " .
			                            "VALUES ( %d , %s , %d , %s ) ON DUPLICATE KEY UPDATE ga_uuid = %s, exactmetrics_cookie = %s";
			$sql                      = $wpdb->prepare( $sql, $transaction_id, $ga_uuid, $order_id, $cookie, $ga_uuid, $cookie );
		} else {
			$exactmetrics_is_in_ga = boolval( $exactmetrics_is_in_ga ) ? 1 : 0;
			$sql                      = "INSERT INTO {$table} ( transaction_id, ga_uuid, order_id, exactmetrics_cookie, exactmetrics_is_in_ga ) " .
			                            "VALUES ( %d , %s , %d , %s, %d ) ON DUPLICATE KEY UPDATE ga_uuid = %s, exactmetrics_cookie = %s , exactmetrics_is_in_ga = %d";
			$sql                      = $wpdb->prepare( $sql, $transaction_id, $ga_uuid, $order_id, $cookie, $exactmetrics_is_in_ga, $ga_uuid, $cookie, $exactmetrics_is_in_ga );
		}
		$res = $wpdb->query( $sql );
		if ( $res === false ) {
	...
	public function mark_refunded_in_transaction_meta( $transaction_id ) {
		...
		$result = $wpdb->update( self::get_table_name(), [ 'exactmetrics_refund_is_in_ga' => 1 ], [ "transaction_id" => $transaction_id ] );
