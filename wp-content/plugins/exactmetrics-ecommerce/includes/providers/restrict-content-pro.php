...
class ExactMetrics_eCommerce_RCP_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_RCP_Integration();
			self::$instance->hooks();
		}
	...
	public function product_detail() {
		...
		if ( ! ExactMetrics_eCommerce_Helper::is_rcp_restricted_content() ) {
			//return;
		}
	...
	public function checkout_page() {
		...
		$rcp_register_page_id = ExactMetrics_eCommerce_Helper::get_rcp_settings( true, 'registration_page' );
		...
		if ( ! function_exists( 'exactmetrics_debug_output' ) && is_user_logged_in() ) {
	...
	public function save_user_cid( $payment_id, $payment_args ) {
		$rcp_payments = ExactMetrics_eCommerce_Helper::rcp_payments();
		$order        = ExactMetrics_eCommerce_Helper::get_rcp_payment( $payment_id );
		...
		if ( ! function_exists( 'exactmetrics_debug_output' ) ) {
			$do_not_track = ! exactmetrics_track_user( $order->user_id );
			if ( $do_not_track ) {
				$rcp_payments->update_meta( $payment_id, '_exactmetrics_ecommerce_do_not_track', true );
			}
		}
		...
		if ( $ga_uuid ) {
			...
			$rcp_payments->update_meta( $payment_id, '_exactmetrics_cookie', $cookie );
		}
	...
	private function track_purchase_v4( $order ) {
		...
		$events = array(
			array(
				...
				'params' => array(
                    ...
                    'session_id'     => get_post_meta( $order->id, '_exactmetrics_ga_session_id', true )
				),
			),
		);
		...
		$args = array(
			'events'    => $events,
			'client_id' => ExactMetrics_eCommerce_Helper::get_rcp_client_id( $order->id ),
		);
	...
	public function add_order( $payment_id ) {
		...
		if ( ExactMetrics_eCommerce_Helper::is_rcp_test_mode( $order->gateway ) ) {
			return;
		}

		$is_in_ga = $rcp_payments->get_meta( $payment_id, '_exactmetrics_is_in_ga', true );
		...
		$rcp_payments->update_meta( $payment_id, '_exactmetrics_is_in_ga', 'yes' );
	}
	...
	public function remove_order( $refund_id ) {
		...
		$is_in_ga = $rcp_payments->get_meta( $refund_id, '_exactmetrics_refund_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_undo_transaction_skip_ga', false, $refund_id );
		if ( $is_in_ga === 'yes' || $skip_ga ) {
			return;
		}

		$order = ExactMetrics_eCommerce_Helper::get_rcp_payment( $refund_id );
		...
		$rcp_payments->update_meta( $refund_id, '_exactmetrics_refund_is_in_ga', 'yes' );
	}
