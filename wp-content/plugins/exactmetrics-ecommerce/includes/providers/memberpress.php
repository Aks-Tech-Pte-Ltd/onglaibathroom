...
class ExactMetrics_eCommerce_MemberPress_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_MemberPress_Integration();
			self::$instance->hooks();
		}
	...
	public function checkout_page( $product_id ) {
		...
		if ( ExactMetrics_eCommerce_Helper::mepr_test_mode() ) {
			return;
		}
	...
	private function track_transaction_v4( $txn, $cid, $discount, $mepr_options ) {
		...
		$events = array(
			array(
				...
				'params' => array(
                    ...
                    'session_id'     => get_post_meta( $txn->id, '_exactmetrics_ga_session_id', true )
				),
			),
		);
		...
		if ( ExactMetrics_eCommerce_Helper::easy_affiliate()->is_easy_affiliate_active() ) {
			...
			$affiliate_id = ExactMetrics_eCommerce_Helper::easy_affiliate()->get_easy_affiliation_woo_affiliate_id( $txn );
		...
		if ( ExactMetrics_eCommerce_Helper::is_affiliate_wp_active() ) {
			...
			$affiliate_id = ExactMetrics_eCommerce_Helper::get_affiliate_wp_affiliate_id( $txn, 'memberpress' );
	...
	public function do_transaction( $txn ) {
		...
		$is_in_ga = $txn->get_meta( '_exactmetrics_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_do_transaction_skip_ga', false, $txn->id );
		...
		// Update in GA
		$txn->update_meta( '_exactmetrics_is_in_ga', 'yes' );
	}
	...
	public function undo_transaction( $txn ) {
		...
		$is_in_ga = $txn->get_meta( '_exactmetrics_refund_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_undo_transaction_skip_ga', false, $txn->id );
		if ( $is_in_ga === 'yes' || $skip_ga ) {
			return;
		}
		...
		$txn->update_meta( '_exactmetrics_refund_is_in_ga', 'yes' );
	}
