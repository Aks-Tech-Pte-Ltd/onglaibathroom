...
class ExactMetrics_eCommerce_EDD_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_EDD_Integration();
			self::$instance->hooks();
		}
	...
	private function hooks() {
		...
		add_filter( 'exactmetrics_frontend_tracking_options_analytics_before_scripts', array(
			$this,
			'require_ec'
		), 10, 1 );
	...
	public function impression( $download_id, $args ) {
		...
		if ( ExactMetrics_eCommerce_Helper::edd_test_mode() ) {
			return;
		}
	...
	public function save_user_cid( $payment_id ) {
		...
		if ( ! function_exists( 'exactmetrics_debug_output' ) ) {
			...
			$do_not_track = ! exactmetrics_track_user( $payment->user_id );
			if ( $do_not_track ) {
	...
	private function track_purchase_v4( $payment_id, $discount, $cart_contents ) {
		...
		$affiliate_id = ExactMetrics_eCommerce_Helper::easy_affiliate()->get_easy_affiliation_edd_affiliate_id( $payment_id );
		...
		if ( ExactMetrics_eCommerce_Helper::is_affiliate_wp_active() ) {
			$affiliate_id = ExactMetrics_eCommerce_Helper::get_affiliate_wp_affiliate_id( $payment_id, 'edd' );
	...
	public function remove_order( $payment_id, $new_status, $old_status ) {
		...
		delete_post_meta( $payment_id, '_exactmetrics_is_in_ga' );
	}
