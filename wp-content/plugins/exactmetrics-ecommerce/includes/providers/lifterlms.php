 ...
 */
class ExactMetrics_eCommerce_LifterLMS_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	 ...
	 * @var string
	 */
	public $cookie_meta_key = '_exactmetrics_cookie';
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_LifterLMS_Integration();
		}
	...
	public function __construct() {
		parent::__construct();
		$this->post_types = apply_filters( 'exactmetrics_llms_post_types', array( 'course', 'llms_membership' ) );
		$this->hooks();
	}
	...
	public function can_user_be_tracked( $uid ) {
		if ( ! function_exists( 'exactmetrics_debug_output' ) ) {
			$do_not_track = ! exactmetrics_track_user( $uid );
			if ( $do_not_track ) {
				return false;
			}
		}
	...
	private function track_purchase_v4( $order, $order_id, $txn ) {
		...
		$events = array(
          ...
          'session_id'     => get_post_meta( $order_id, '_exactmetrics_ga_session_id', true )
				),
			),
		);
	...
	public function add_order( $txn ) {
		...
		if ( ExactMetrics_eCommerce_Helper::llms_test_mode() ) {
			return;
		}
		...
		$is_in_ga = get_post_meta( $order_id, '_exactmetrics_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_do_transaction_skip_ga', false, $order_id );
		...
		update_post_meta( $order_id, '_exactmetrics_is_in_ga', 'yes' );
	}

	private function can_track_order( $order, $txn_id ) {
		// If we're skipping renewals we must check if this is a renewal.
		if ( $order->is_recurring() && apply_filters( 'exactmetrics_ecommerce_skip_renewals', true ) ) {
	...
	public function remove_order( $txn ) {
		...
		$is_in_ga = get_post_meta( $order_id, '_exactmetrics_refund_is_in_ga', true );
		...
		update_post_meta( $order_id, '_exactmetrics_refund_is_in_ga', 'yes' );

		update_post_meta( $txn_id, '_exactmetrics_refund_is_in_ga', 'yes' );
	}
	...
	protected function get_cart_items( $plan ) {
		...
		return array(
			...
			'pr1pr' => ExactMetrics_eCommerce_Helper::round_price( $price ), // Product Price
	...
	protected function get_list_type() {
		...
		return apply_filters( 'exactmetrics_llms_get_list_type', $list_type );
	}
	...
	public function save_user_cid( $order ) {
		...
		// Skip tracking?
		if ( ! $this->can_user_be_tracked( $order->get( 'user_id' ) ) ) {
			update_post_meta( $order_id, '_exactmetrics_ecommerce_do_not_track', true );
		}
		...
		if ( $ga_uuid ) {
			...
			update_post_meta( $order_id, '_exactmetrics_cookie', $cookie );
		}
