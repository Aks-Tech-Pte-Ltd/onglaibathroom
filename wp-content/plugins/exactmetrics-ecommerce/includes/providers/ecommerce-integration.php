...
class ExactMetrics_Enhanced_eCommerce_Integration {
	...
	/**
	 * ExactMetrics_Enhanced_eCommerce_Integration constructor.
	 */
	...
	protected function get_tracking_mode() {
		...
		if ( method_exists( ExactMetrics(), 'get_tracking_mode' ) ) {
			$tracking = ExactMetrics()->get_tracking_mode();
		}
	...
	protected function generic_hooks() {
		...
		add_filter( 'exactmetrics_tracking_after_gtag', array( $this, 'print_dual_tracking_js' ), 11, 1 );
	...
	public function print_dual_tracking_js() {
		$v4_id = exactmetrics_get_v4_id_to_output();  // phpcs:ignore
	...
	public function print_impressions_js( $options ) {
		...
		$attr_string = function_exists( 'exactmetrics_get_frontend_analytics_script_atts' ) ? exactmetrics_get_frontend_analytics_script_atts() : ' type="text/javascript" data-cfasync="false"';
    ...
    protected function save_user_session_id( $payment_id, $measurement_id ) {
        if ( function_exists( 'exactmetrics_get_browser_session_id' ) ) {
            ...
            update_post_meta($payment_id, '_exactmetrics_ga_session_id', $session_id);
        }
    }
}
