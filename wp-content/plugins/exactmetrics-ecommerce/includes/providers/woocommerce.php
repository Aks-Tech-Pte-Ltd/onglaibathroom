...
class ExactMetrics_eCommerce_WooCommerce_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_WooCommerce_Integration();
			self::$instance->hooks();
		}
	...
	public function impression() {
		...
		if ( ExactMetrics_eCommerce_Helper::woocommerce_test_mode() ) {
			return;
		}
		...
		$data = array(
			...
			'variant'  => $variation,
			'position' => $position,
			'price'    => ExactMetrics_eCommerce_Helper::round_price( $product->get_price() ),
		);
	...
	private function track_add_to_cart_v4( $product_id, $variation_id, $quantity ) {
		if (
			! function_exists( 'exactmetrics_get_v4_id_to_output' ) ||
	...
	public function save_user_cid( $payment_id ) {
		...
		if ( ! function_exists( 'exactmetrics_debug_output' ) ) {
			...
			$do_not_track = ! exactmetrics_track_user( $order->get_user_id() );
			if ( $do_not_track ) {
				update_post_meta( $payment_id, '_exactmetrics_ecommerce_do_not_track', true );
			}
		}
		...
		if ( $ga_uuid ) {
			...
			update_post_meta( $payment_id, '_exactmetrics_cookie', $cookie );
		}

        if ( $measurement_id = exactmetrics_get_v4_id_to_output() ) {
	...
	private function track_purchase_v4( $order, $payment_id, $discount ) {
		...
		$events = array(
			array(
				...
				'params' => array(
					...
					'items'          => $items,
                    'session_id'     => get_post_meta( $payment_id, '_exactmetrics_ga_session_id', true )
				),
			),
		);

		if ( ExactMetrics_eCommerce_Helper::easy_affiliate()->is_easy_affiliate_active() ) {

			$affiliate_id = ExactMetrics_eCommerce_Helper::easy_affiliate()->get_easy_affiliation_woo_affiliate_id( $payment_id );
		...
		if ( ExactMetrics_eCommerce_Helper::is_affiliate_wp_active() ) {

			$affiliate_id = ExactMetrics_eCommerce_Helper::get_affiliate_wp_affiliate_id( $payment_id, 'woocommerce' );
		...
		$args = array(
			...
			'client_id'  => exactmetrics_get_client_id( $payment_id ),
			'events'     => $events,
		);
	...
	public function add_order( $payment_id ) {
		...
		// Don't track test mode.
		if ( ExactMetrics_eCommerce_Helper::woocommerce_test_mode( $payment_id ) ) {
			return;
		}

		$is_in_ga = get_post_meta( $payment_id, '_exactmetrics_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_do_transaction_skip_ga', false, $payment_id );
		...
		update_post_meta( $payment_id, '_exactmetrics_is_in_ga', 'yes' );
	}
	...
	private function track_refund_v4( $order_id, $refund_id, $is_total_refund ) {
		...
		$args = array(
			'events'    => $events,
			'client_id' => exactmetrics_get_client_id( $order_id ),
		);
	...
	public function remove_order( $order_id, $refund_id, $is_total_refund = false ) {
		...
		$is_in_ga = get_post_meta( $refund_id, '_exactmetrics_refund_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_undo_transaction_skip_ga', false, $order_id );
		if ( $is_in_ga === 'yes' || $skip_ga ) {
			return;
		}
		...
		update_post_meta( $refund_id, '_exactmetrics_refund_is_in_ga', 'yes' );
	}
