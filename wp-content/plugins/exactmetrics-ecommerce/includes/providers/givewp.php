 ...
 * @since 7.4.0
 */
class ExactMetrics_eCommerce_GiveWP_Integration extends ExactMetrics_Enhanced_eCommerce_Integration {
	...
	public static function get_instance() {
		if ( ! self::$instance ) {
			self::$instance = new ExactMetrics_eCommerce_GiveWP_Integration();
			self::$instance->hooks();
		}
	...
	public function impression() {
		...
		if ( ExactMetrics_eCommerce_Helper::givewp_is_test_mode() ) {
			return;
		}
	...
	public function save_user_cid( $payment_id, $payment_data ) {
		...
		if ( ! function_exists( 'exactmetrics_debug_output' ) ) {
			if ( ! give_is_guest_payment( $payment_id ) ) {
				...
				$do_not_track = ! exactmetrics_track_user( $donor_id );
				if ( $do_not_track ) {
					give_update_payment_meta( $payment_id, '_exactmetrics_ecommerce_do_not_track', true );
				}
			}
		}
		...
		if ( $ga_uuid ) {
			...
			give_update_payment_meta( $payment_id, '_exactmetrics_cookie', $cookie );
		}
    ...
    protected function save_user_session_id( $payment_id, $measurement_id )
    {
        if ( function_exists( 'exactmetrics_get_browser_session_id' ) ) {
            ...
            give_update_payment_meta($payment_id, '_exactmetrics_ga_session_id', $session_id);
        }
    }
	...
	public function add_order( $payment_id ) {
		...
		$is_in_ga = give_get_payment_meta( $payment_id, '_exactmetrics_is_in_ga', true );
		...
		$donation_id = ExactMetrics_eCommerce_Helper::givewp_donation_id( $payment_id );

		$this->track_purchase_v4( $donation_id, $total, $payment_id, $currency, $donor_id );

		give_update_payment_meta( $payment_id, '_exactmetrics_is_in_ga', 'yes' );
	}
	...
	public function remove_order( $payment_id, $new_status, $old_status ) {
		...
		$negative_statuses = ExactMetrics_eCommerce_Helper::givewp_negative_statutes();
		...
		$is_in_ga = give_get_payment_meta( $payment_id, '_exactmetrics_refund_is_in_ga', true );
		$skip_ga  = apply_filters( 'exactmetrics_ecommerce_undo_transaction_skip_ga', false, $payment_id );
		if ( $is_in_ga === 'yes' || $skip_ga ) {
			return;
		}
		...
		give_update_payment_meta( $payment_id, '_exactmetrics_refund_is_in_ga', 'yes' );
	}
	...
	public function get_list_type( $product_id = 0 ) {
		...
		return apply_filters( 'exactmetrics_ga_ecommerce_get_list_type', $list_type );
	}
