 ...
 * @since 8.0.0
 */
class ExactMetrics_Ecommerce_Compatibility_Check {
	/**
	 * Holds singleton instance
	 *
	 * @since 8.0.0
	 * @var ExactMetrics_Ecommerce_Compatibility_Check
	 */
	 ...
	 *
	 * @return ExactMetrics_Ecommerce_Compatibility_Check
	 * @since 8.0.0
	 */
	...
	private function __construct() {
		add_filter( 'exactmetrics_compatible_php_version', array( $this, 'filter_compatible_php_version' ), 10, 1 );
		add_filter( 'exactmetrics_compatible_wp_version', array( $this, 'filter_compatible_wp_version' ), 10, 1 );
	}
	...
	public function get_compatible_php_version() {
		return apply_filters( 'exactmetrics_compatible_php_version', $this->compatible_php_version );
	}
	...
	public function get_compatible_wp_version() {
		return apply_filters( 'exactmetrics_compatible_wp_version', $this->compatible_wp_version );
	}
	...
	private function is_notice_already_active() {
		return defined( 'EXACTMETRICS_VERSION_NOTICE_ACTIVE' ) && EXACTMETRICS_VERSION_NOTICE_ACTIVE;
	}
	...
	private function set_notice_active() {
		if ( ! defined( 'EXACTMETRICS_VERSION_NOTICE_ACTIVE' ) ) {
			define( 'EXACTMETRICS_VERSION_NOTICE_ACTIVE', true );
		}
	}
	...
	public function display_php_notice() {
		...
		<div class="error">
			<p>
				...
				echo sprintf(
					esc_html__( 'Sorry, but your current version of PHP does not meet the minimum required version %1$s%2$s%3$s or newer to run ExactMetrics Ecommerce Addon properly. For information on how to upgrade your PHP version, contact your web host. %4$sClick here to return to the Dashboard%5$s.', 'exactmetrics-ecommerce' ),
					'<strong>',
	...
	public function display_wp_notice() {
		$url = exactmetrics_get_url( 'notice', 'notice-wrong-wp-version', 'https://www.wpbeginner.com/beginners-guide/ultimate-guide-to-upgrade-wordpress-for-beginners-infograph/' );
		...
		<div class="error">
			<p>
				...
				echo sprintf(
					esc_html__( 'Sorry, but your WordPress version is not %1$s%2$s%3$s or newer. Please update your WordPress version and then activate ExactMetrics Ecommerce Addon. For help on how to update your WordPress %4$sclick here%5$s.', 'exactmetrics-ecommerce' ),
					'<strong>',
