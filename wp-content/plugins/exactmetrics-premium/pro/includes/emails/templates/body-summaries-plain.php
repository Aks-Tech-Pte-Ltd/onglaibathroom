...
$visitors_difference = sprintf( __( 'Decrease visitors %s', 'exactmetrics-premium' ), $prev_visitors_percentage );
if ( (int) $prev_visitors_percentage === (int) $prev_visitors_percentage && (int) $prev_visitors_percentage >= 0 ) {
	// Translators: Placeholder adds the percentage of visitors.
	$visitors_difference = sprintf( __( 'Increase visitors %s', 'exactmetrics-premium' ), $prev_visitors_percentage );
}
...
$pageviews_difference = sprintf( __( 'Decrease pageviews %s', 'exactmetrics-premium' ), $prev_pageviews_percentage );
if ( (int) $prev_pageviews_percentage === (int) $prev_pageviews_percentage && (int) $prev_pageviews_percentage >= 0 ) {
	// Translators: Placeholder adds the percentage of pageviews.
	$pageviews_difference = sprintf( __( 'Increase pageviews %s', 'exactmetrics-premium' ), $prev_pageviews_percentage );
}
...
echo esc_html__( 'Total Visitors', 'exactmetrics-premium' ) . '   -   ' . esc_html( number_format_i18n( $total_visitors ) ) . "\n";
echo esc_html( $visitors_difference ) . "%\n\n";

echo esc_html__( 'Total Pageviews', 'exactmetrics-premium' ) . '   -   ' . esc_html( number_format_i18n( $total_pageviews ) ) . "\n";
...
if ( isset( $info_block['title'] ) && ! empty( $info_block['title'] ) ) {
	echo esc_html__( 'Pro Tip from our experts', 'exactmetrics-premium' ) . "\n\n";
...
echo esc_html__( "To make sure you keep getting these emails, please add support@exactmetrics.com to your address book or whitelist us. Want out of the loop? Unsubscribe ", "exactmetrics-premium" ) . $settings_tab_url . "\n\n"; // phpcs:ignore
