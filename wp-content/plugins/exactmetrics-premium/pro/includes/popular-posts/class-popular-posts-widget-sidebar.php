...
class ExactMetrics_Popular_Posts_Widget_Sidebar extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo ExactMetrics_Popular_Posts_Widget()->shortcode_output( $atts ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function form( $instance ) {
		...
		$meta_font_sizes  = apply_filters( 'exactmetrics_popular_posts_widget_meta_sizes', range( 8, 24 ) );
		...
		<div class="exactmetrics-widget-theme-preview">
			...
			<div
				class="exactmetrics-widget-theme-preview-icon exactmetrics-widget-theme-preview-icon-<?php echo esc_attr( $instance['theme'] ); ?>"></div>
		</div>
		...
		if ( ! empty( $theme['styles']['meta']['color'] ) ) {
			$this->color_input( 'meta_color', _x( 'Meta Color:', 'Widget', 'google-analytics-dashboard-for-wp' ), $instance );
		}
		if ( ! empty( $theme['styles']['meta']['size'] ) ) {
			$this->size_input( 'meta_size', _x( 'Meta Font Size:', 'Widget', 'google-analytics-dashboard-for-wp' ), $instance, $meta_font_sizes );
		}
		...
		<p>
			<?php if ( ! empty( $theme['styles']['meta']['author'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Author', 'google-analytics-dashboard-for-wp' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['date'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Date', 'google-analytics-dashboard-for-wp' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['comments'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Comments', 'google-analytics-dashboard-for-wp' ); ?>
				</label>
			<?php } ?>
		</p>
		<?php
		...
		<p>
			...
			<select id="<?php echo esc_attr( $this->get_field_id( $name ) ); ?>"
				...
				<?php foreach ( $range as $font_size ) { ?>
					<option
						value="<?php echo absint( $font_size ); ?>" <?php selected( $instance[ $name ], $font_size ); ?>><?php printf( esc_html_x( '%dpx', 'google-analytics-dashboard-for-wp' ), $font_size ); // phpcs:ignore ?></option>
				<?php } ?>
			</select>
		</p>
		<?php
	...
	public function load_widget_scripts() {
		...
		wp_register_script( 'exactmetrics-select2', plugins_url( 'pro/assets/js/select2.min.js', EXACTMETRICS_PLUGIN_FILE ), array(
			'jquery',
		), '4.0.13', true );
