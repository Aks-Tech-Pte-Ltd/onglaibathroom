...
class ExactMetrics_Popular_Posts_Widget extends ExactMetrics_Popular_Posts {
	...
	public function get_rendered_html( $atts ) {
		...
		if ( isset( $theme_styles['styles']['meta']['author'] ) ) {
			...
			if ( isset( $atts['meta_author'] ) ) {
				$show_author = 'on' === esc_html($atts['meta_author']);
			}
		}
		...
		if ( isset( $theme_styles['styles']['meta']['date'] ) ) {
			...
			if ( isset( $atts['meta_date'] ) ) {
				$show_date = 'on' === esc_html($atts['meta_date']);
			}
		}
	...
	public function build_inline_styles() {
		...
		foreach ( $themes as $theme_key => $theme_styles ) {
			...
			if ( ! empty( $theme_styles['meta'] ) ) {
				$styles .= '.exactmetrics-popular-posts-styled.exactmetrics-widget-popular-posts.exactmetrics-widget-popular-posts-' . esc_attr($theme_key) . ' .exactmetrics-widget-popular-posts-list .exactmetrics-widget-popular-posts-meta span {';

				if ( ! empty( $theme_styles['meta']['color'] ) ) {
					$styles .= 'color:' . esc_attr($theme_styles['meta']['color']) . ';';
				}
				if ( ! empty( $theme_styles['meta']['size'] ) ) {
					$styles .= 'font-size:' . esc_attr($theme_styles['meta']['size']) . 'px;';
				}
			...
			if ( ! empty( $theme_styles['comments'] ) ) {
				$styles .= '.exactmetrics-widget-popular-posts.exactmetrics-popular-posts-styled.exactmetrics-widget-popular-posts-' . esc_attr($theme_key) . ' .exactmetrics-widget-popular-posts-list .exactmetrics-widget-popular-posts-comments {';

				if ( ! empty( $theme_styles['comments']['color'] ) ) {
					$styles .= 'color:' . esc_attr($theme_styles['comments']['color']) . ';';
				}

				$styles .= '}';
				$styles .= '.exactmetrics-widget-popular-posts.exactmetrics-popular-posts-styled.exactmetrics-widget-popular-posts-' . esc_attr($theme_key) . ' .exactmetrics-widget-popular-posts-list .exactmetrics-widget-popular-posts-comments svg {';

				if ( ! empty( $theme_styles['comments']['color'] ) ) {
					$styles .= 'fill:' . esc_attr($theme_styles['comments']['color']) . ';';
				}
			...
			if ( ! empty( $theme_styles['border']['color2'] ) ) {
				$styles .= '.exactmetrics-popular-posts-styled.exactmetrics-widget-popular-posts-' . esc_attr($theme_key) . ' .exactmetrics-inline-popular-posts-border-2 {';

				$styles .= 'border-color:' . esc_attr($theme_styles['border']['color2']) . ';';
	...
	protected function get_query_args_curated() {
		...
		if ( class_exists( 'ExactMetrics_Popular_Posts_GA' ) && $this->ga ) {
			$popular_posts_ga = ExactMetrics_Popular_Posts_GA::get_instance()->get_popular_posts();
