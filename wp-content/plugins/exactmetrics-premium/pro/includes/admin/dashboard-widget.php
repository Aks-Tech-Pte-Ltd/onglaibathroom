 ...
 */
class ExactMetrics_Dashboard_Widget_Pro {
	...
	/**
	 * ExactMetrics_Dashboard_Widget_Pro constructor.
	 */
...
//		add_action( 'admin_footer', array( $this, 'load_notice' ) );
	...
	public function register_dashboard_widget() {
		...
		wp_add_dashboard_widget(
			self::WIDGET_KEY,
			esc_html__( 'ExactMetrics', 'exactmetrics-premium' ),
	...
	public function dashboard_widget_content() {
		...
		$not_licensed = ( ! ExactMetrics()->license->is_site_licensed() && ! ExactMetrics()->license->is_network_licensed() );
	...
	public function widget_content_no_auth() {
		...
		if ( ! ExactMetrics()->license->site_license_expired() && ! ExactMetrics()->license->network_license_expired() ) {
			?>
			<div class="mi-dw-not-authed">
				<h2><?php esc_html_e( 'Website Analytics is not Setup', 'exactmetrics-premium' ); ?></h2>
				<?php if ( current_user_can( 'exactmetrics_save_settings' ) ) { ?>
					<p><?php esc_html_e( 'To see your website stats, please connect ExactMetrics to Google Analytics.', 'exactmetrics-premium' ); ?></p>
					<a href="<?php echo esc_url( $url ); ?>"
					   class="mi-dw-btn-large"><?php esc_html_e( 'Setup Website Analytics', 'exactmetrics-premium' ); ?></a>
				<?php } else { ?>
					<p><?php esc_html_e( 'To see your website stats, please ask your webmaster to connect ExactMetrics to Google Analytics.', 'exactmetrics-premium' ); ?></p>
				<?php } ?>
			</div>
			<?php
		} else {
			...
			<div class="mi-dw-license-expired">
				<div class="exactmetrics-expired-license-icon">
				...
				<h2><?php echo sprintf( esc_html__( 'Your ExactMetrics %1$s License has expired.', 'exactmetrics-premium' ), ucfirst( $type ) ); // phpcs:ignore ?></h2>
				<?php if ( current_user_can( 'exactmetrics_save_settings' ) ) { ?>
					<p><?php esc_html_e( 'To ensure your site continues to track, view reports, and receive new updates, you must reactivate your license.', 'exactmetrics-premium' ); ?></p>
					<a target="_blank" href="<?php echo esc_url( $reactivation_url ); ?>"
					   class="mi-dw-btn-large"><?php esc_html_e( 'Reactivate License', 'exactmetrics-premium' ); ?></a>
				<?php } else { ?>
					<p><?php esc_html_e( 'To ensure your site continues to track, view reports, and receive new updates, please ask your webmaster to reactivate the license.', 'exactmetrics-premium' ); ?></p>
				<?php } ?>
			</div>
			<?php
		}
