 ...
 * @since 7.12.3
 */
final class ExactMetrics_Notification_To_Track_Categories extends ExactMetrics_Notification_Event {

	public $notification_id             = 'exactmetrics_notification_to_track_categories';
	...
	public function prepare_notification_data( $notification ) {
		...
		if ( false === $is_category_tracking_enabled || false === $is_tags_tracking_enabled || ( is_array( $custom_dimensions ) && empty( $custom_dimensions ) ) ) {
			...
			$notification['title'] = __( 'Track WordPress Categories and Tags in Google Analytics', 'exactmetrics-premium' );
			// Translators: track categories notification content
			if ( ! $is_em ) {
				$notification['content'] = sprintf( __( 'Do you want to know which WordPress categories and tags are the most popular on your site? By default, Google Analytics doesn’t track them, but there’s an easy way to track WordPress tags and categories. <br><br>All you need to do is set up the Custom Dimensions settings in ExactMetrics and Google Analytics. Detailed instructions on how to set up categories and tags tracking are available in %1$sthis article.%2$s', 'exactmetrics-premium' ), '<a href="' . $this->build_external_link( 'https://www.exactmetrics.com/the-beginners-guide-to-custom-dimensions-in-google-analytics/' ) . '" target="_blank">', '</a>' );
			} else {
				$notification['content'] = esc_html__( 'Do you want to know which WordPress categories and tags are the most popular on your site? By default, Google Analytics doesn’t track them, but there’s an easy way to track WordPress tags and categories. <br><br>All you need to do is set up the Custom Dimensions settings in ExactMetrics and Google Analytics.', 'exactmetrics-premium' );
			}

			if ( ! $is_em ) {
				$notification['btns'] = array(
					'learn_more' => array(
						'url'         => $this->build_external_link( 'https://www.exactmetrics.com/the-beginners-guide-to-custom-dimensions-in-google-analytics/' ),
...
new ExactMetrics_Notification_To_Track_Categories();