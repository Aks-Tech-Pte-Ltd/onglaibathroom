...
final class ExactMetrics_Notification_Connect_Search_Console extends ExactMetrics_Notification_Event {
	...
	public function prepare_notification_data( $notification ) {
		...
		if ( isset( $report['success'] ) && false === $report['success'] && ! empty( $report['error'] ) ) {
			...
			if ( ! $is_em ) {
				$notification['content'] = sprintf( __( 'Are you interested in what keywords bring you the most traffic from Google? You can get that information directly in your ExactMetrics Reports area by connecting your Google Search Console account with Google Analytics. <br><br>Follow our %1$sstep-by-step guide%2$s to get started and find out where to focus your attention.', 'exactmetrics-premium' ), '<a href="' . $this->build_external_link( 'https://www.exactmetrics.com/how-to-track-keywords-in-google-analytics/' ) . '" target="_blank">', '</a>' );
			} else {
				$notification['content'] = esc_html__( 'Are you interested in what keywords bring you the most traffic from Google? You can get that information directly in your ExactMetrics Reports area by connecting your Google Search Console account with Google Analytics.', 'exactmetrics-premium' );
			}
