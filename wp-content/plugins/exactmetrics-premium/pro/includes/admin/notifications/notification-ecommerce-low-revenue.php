 ...
 * @since 7.12.3
 */
final class ExactMetrics_Notification_Ecommerce_Low_Revenue extends ExactMetrics_Notification_Event {

	public $notification_id = 'exactmetrics_notification_ecommerce_low_revenue';
	...
	public function prepare_notification_data( $notification ) {
        ...
        if ( ! empty( $data ) && $data['prev_revenue_difference'] < 0 ) {
            ...
            $notification['title'] = sprintf( __( 'Your eCommerce revenue has dropped to $%s', 'exactmetrics-premium' ), $data['revenue'] );
            // Translators: low revenue notification content
            $notification['content'] = sprintf( __( 'Your eCommerce revenue decreased compared to the previous 30 days. Is your online store performing as well as it could? Unless every single person who visits your store makes a purchase, there’s always room for improvement, and eCommerce optimization may not be as much work as you think. <br><br>If you know where to look, just a few small tweaks can mean a 50%% increase in sales (or even more). To boost revenue, take a look at the guidelines in %sthis article%s.', 'exactmetrics-premium' ), '<a href="' . $this->build_external_link( 'https://optinmonster.com/ecommerce-optimization-guide/' ) . '" target="_blank">', '</a>' );
            $notification['btns']    = array(
                "view_ecommerce_report" => array(
                    'url'  => $this->get_view_url( 'exactmetrics-report-ecommerce-revenue', 'exactmetrics_reports', 'ecommerce' ),
...
new ExactMetrics_Notification_Ecommerce_Low_Revenue();
