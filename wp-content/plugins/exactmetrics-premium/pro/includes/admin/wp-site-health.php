 ...
 */
class ExactMetrics_WP_Site_Health_Pro {
	...
	public function is_tracking() {
		...
		if ( ! isset( $this->is_tracking ) ) {
			...
			$this->is_tracking = ! empty( $tracking_tag );
		}
	...
	public function test_check_authentication() {
		...
		if ( ! $this->is_authed ) {
			if ( '' !== exactmetrics_get_v4_id() ) {
				...
				$result['label']       = __( 'You are using Manual GA4 Measurement ID output', 'exactmetrics-premium' );
...
new ExactMetrics_WP_Site_Health_Pro();
