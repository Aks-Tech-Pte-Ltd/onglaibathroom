...
final class ExactMetrics_Report_RealTime extends ExactMetrics_Report {

	public $title;
	public $class = 'ExactMetrics_Report_RealTime';
	...
	public function __construct() {
		$this->title = __( 'Real-Time', 'exactmetrics-premium' );
		add_filter( 'exactmetrics_report_transient_expiration', array( $this, 'sixty_seconds' ), 10, 2 );
		add_filter( 'exactmetrics_report_use_cache', array( $this, 'use_cache' ), 10, 2 );
		parent::__construct();
	}
	...
	public function prepare_report_data( $data ) {
		if ( ! empty( $data['data']['realtime'] ) ) {
			foreach ( $data['data']['realtime']['pageviewsovertime']['labels'] as $key => $label ) {
				...
				$data['data']['realtime']['pageviewsovertime']['labels'][ $key ] = sprintf( esc_html__( '%s minutes ago', 'exactmetrics-premium' ), $label );
			}

			// Add countries data.
			$countries            = array_flip( exactmetrics_get_country_list( false ) );
			$countries_translated = exactmetrics_get_country_list( true );
		...
		if ( ! empty( $data['data'] ) ) {
			$data['data']['galinks'] = array(
				'toppages'  => $this->get_ga_report_url(
					'',
					$data['data'],
					'',
					'overview',
				...
				'referrals' => $this->get_ga_report_url(
					'',
					$data['data'],
					'',
					'overview',
				...
				'countries' => $this->get_ga_report_url(
					'',
					$data['data'],
					'',
					'overview',
