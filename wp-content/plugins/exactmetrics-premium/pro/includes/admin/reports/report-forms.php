...
final class ExactMetrics_Report_Forms extends ExactMetrics_Report {

	public $title;
	public $class = 'ExactMetrics_Report_Forms';
	public $name = 'forms';
	...
	public function __construct() {
		$this->title = __( 'Forms', 'exactmetrics-premium' );
		parent::__construct();
	}
	...
	public function requirements( $error = false, $args = array(), $name = '' ) {
		...
		if ( ! class_exists( 'ExactMetrics_Forms' ) ) {
			...
			$text = __( 'Please %s the ExactMetrics Forms addon to view custom dimensions reports.', 'exactmetrics-premium' );
	...
	public function prepare_report_data( $data ) {
		...
		if ( ! empty( $data['data'] ) ) {
			$data['data']['galinks'] = array(
				'forms' => $this->get_ga_report_url(
					'events-overview',
					$data['data'],
					'_r..dimension-value={"dimension":"eventName","value":"generate_lead"}',
					'dashboard'
				),
			);
		}
