...
final class ExactMetrics_Report_Dimensions extends ExactMetrics_Report {

	public $title;
	public $class = 'ExactMetrics_Report_Dimensions';
	...
	public function __construct() {
		$this->title = __( 'Dimensions', 'exactmetrics-premium' );
		parent::__construct();
	}
	...
	public function requirements( $error = false, $args = array(), $name = '' ) {
		...
		if ( ! class_exists( 'ExactMetrics_Admin_Custom_Dimensions' ) ) {
			...
			$text = __( 'Please %s the ExactMetrics Dimensions addon to view custom dimensions reports.', 'exactmetrics-premium' );
		...
		if ( empty( $dimensions ) || ! is_array( $dimensions ) ) {
			add_filter( 'exactmetrics_reports_handle_error_message', array(
				$this,
				'add_dimensions_settings_link'
			) );

			return __( 'Please enable at least 1 dimension to use this report', 'exactmetrics-premium' );
		}
	...
	public function additional_data() {
		$dimensions       = $this->get_active_enabled_custom_dimensions();
		$dimension_params = wp_list_pluck(
			$dimensions,
			'key'
		);

		return array( 'count' => count( $dimensions ), 'dimension_parameters' => $dimension_params );
	}
	...
	public function get_active_enabled_custom_dimensions() {
		$dimensions                = array();
		$custom_dimensions         = new ExactMetrics_Admin_Custom_Dimensions();
		$default_custom_dimensions = $custom_dimensions->custom_dimensions();
		$custom_dimension_option   = exactmetrics_get_option( 'custom_dimensions', array() );
	...
	public function prepare_report_data( $data ) {
		...
		if ( class_exists( 'ExactMetrics_Admin_Custom_Dimensions' ) ) {
			$custom_dimensions_option = exactmetrics_get_option( 'custom_dimensions', array() );
				...
				if ( intval( $key ) > 0 ) {
					foreach ( $data['data'][ $key ]['data'] as $dimension_key => $dimension_info ) {
						...
						if ( 'true' === $title ) {
							...
							$title = __( 'Logged-Out', 'exactmetrics-dimensions' );
						}
						if ( is_string( $title ) &&
						     preg_match( '/^' .
						...
						if ( intval( $user_id_dimension_index ) === intval( $key ) ) {
							...
							$title    = apply_filters( 'exactmetrics_dimensions_user_id_display', $username, $user_id );
						}
	...
	public function add_dimensions_settings_link( $data ) {
		if ( current_user_can( 'exactmetrics_save_settings' ) ) {
			$data['data']['footer'] = sprintf(
				'<a href="%s">%s</a>',
				admin_url( 'admin.php?page=exactmetrics_settings#/conversions?section=dimensions' ),
				esc_html__( 'View dimensions settings', 'exactmetrics-premium' )
			);
		}
