...
final class ExactMetrics_License_Actions {
	/**
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_exactmetrics_verify_license', array( $this, 'ajax_verify_key' ) );

		add_action( 'wp_ajax_exactmetrics_deactivate_license', array( $this, 'ajax_deactivate_key' ) );
		add_action(
			'wp_ajax_exactmetrics_deactivate_expired_license',
			array(
				$this,
		...
		add_action( 'wp_ajax_exactmetrics_validate_license', array( $this, 'ajax_validate_key' ) );
	}
	...
	public function admin_init() {
		...
		add_action( 'admin_notices', array( $this, 'exactmetrics_notices' ), 11 );
		add_action( 'network_admin_notices', array( $this, 'exactmetrics_notices' ), 11 );

		// Grab the license key. If it is not set (even after verification), return early.
		$this->key = is_network_admin() ? ExactMetrics()->license->get_network_license_key() : ExactMetrics()->license->get_site_license_key();
		if ( ! $this->key ) {
			return;
		}
	...
	public function maybe_verify_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || strlen( $_POST['exactmetrics-license-key'] ) < 10 || empty( $_POST['exactmetrics-verify-submit'] ) ) { // phpcs:ignore
			return;
		}

		if ( ! wp_verify_nonce( $_POST['exactmetrics-key-nonce'], 'exactmetrics-key-nonce' ) ) { // phpcs:ignore
			return;
		}
	...
	public function ajax_verify_key() {
		...
		if ( ! $license ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Please enter a license key.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'You are not allowed to verify a license key.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		if ( ! $verify ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		$option                = $network_admin ? ExactMetrics()->license->get_network_license() : ExactMetrics()->license->get_site_license();
		$option['key']         = $license;
		...
		$network_admin ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
		delete_transient( '_exactmetrics_addons' );
		exactmetrics_get_addons_data( $option['key'] );

		$message = isset( $verify->success ) ? $verify->success : esc_html__( 'Congratulations! This site is now receiving automatic updates.', 'google-analytics-dashboard-for-wp' );
	...
	public function verify_key( $key = '' ) {
		...
		if ( empty( $key ) && ! empty( $_POST['exactmetrics-license-key'] ) ) {
			$key = $_POST['exactmetrics-license-key']; // phpcs:ignore
		}
		...
		if ( ! $verify ) {
			$this->errors[] = esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'google-analytics-dashboard-for-wp' );
		...
		// Otherwise, our request has been done successfully. Update the option and set the success message.
		$option                = is_network_admin() ? ExactMetrics()->license->get_network_license() : ExactMetrics()->license->get_site_license();
		$option['key']         = trim( $key );
		$option['type']        = isset( $verify->type ) ? $verify->type : $option['type'];
		$option['is_expired']  = false;
		$option['is_disabled'] = false;
		$option['is_invalid']  = false;
		$this->success[]       = isset( $verify->success ) ? $verify->success : esc_html__( 'Congratulations! This site is now receiving automatic updates.', 'google-analytics-dashboard-for-wp' );
		is_network_admin() ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
		delete_transient( '_exactmetrics_addons' );
		exactmetrics_get_addons_data( $option['key'] );
	...
	public function maybe_validate_key() {
		$check = is_network_admin() ? ExactMetrics()->license->time_to_check_network_license() : ExactMetrics()->license->time_to_check_site_license();
		if ( $check ) {
			$this->validate_key();
		}
	}
	...
	public function validate_key( $forced = false, $network = null ) {
		...
		$option                = $network ? ExactMetrics()->license->get_network_license() : ExactMetrics()->license->get_site_license();
		$option['is_expired']  = false;
		$option['expiry_date'] = '';
		$option['is_disabled'] = false;
		$option['is_invalid']  = false;
		...
		if ( isset( $validate->key ) || isset( $validate->author ) ) {
			...
			$network ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
			$this->errors[] = esc_html__( 'Your license key for ExactMetrics is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'google-analytics-dashboard-for-wp' );
		...
		if ( isset( $validate->expired ) ) {
			...
			$network ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
			// Translators: The link to the ExactMetrics website.
		...
		if ( isset( $validate->disabled ) ) {
			...
			$network ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
			$this->errors[] = esc_html__( 'Your license key for ExactMetrics has been disabled. Please use a different key.', 'google-analytics-dashboard-for-wp' );
		...
		if ( ( ! empty( $validate->key ) || ! empty( $this->key ) ) && $forced ) {
			...
			delete_transient( '_exactmetrics_addons' );
			exactmetrics_get_addons_data( $key );
			$this->success[] = esc_html__( 'Congratulations! Your key has been refreshed successfully.', 'google-analytics-dashboard-for-wp' );
		}
	...
	public function maybe_deactivate_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || empty( $_POST['exactmetrics-deactivate-submit'] ) ) { // phpcs:ignore
			return;
		}
	...
	public function ajax_deactivate_key() {
		...
		if ( ! $license ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Please refresh the page and try again.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'You are not allowed to deactivate the license key.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		$network_admin ? ExactMetrics()->license->delete_network_license() : ExactMetrics()->license->delete_site_license();

		wp_send_json_success(
			array(
				'message' => isset( $deactivate->success ) ? $deactivate->success : esc_html__( 'Congratulations! You have deactivated the key from this site successfully.', 'google-analytics-dashboard-for-wp' ),
			)
		);
	}
	...
	public function deactivate_key() {
		...
		$deactivate = $this->perform_remote_request( 'deactivate-key', array( 'tgm-updater-key' => $_POST['exactmetrics-license-key'] ) ); // phpcs:ignore
		...
		// If an error is returned, set the error and return.
		if ( ! empty( $deactivate->error ) && ! exactmetrics_is_debug_mode() ) {
		...
		$this->success[] = isset( $deactivate->success ) ? $deactivate->success : esc_html__( 'Congratulations! You have deactivated the key from this site successfully.', 'google-analytics-dashboard-for-wp' );
		is_network_admin() ? ExactMetrics()->license->delete_network_license() : ExactMetrics()->license->delete_site_license();
	}
	...
	public function maybe_refresh_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || empty( $_POST['exactmetrics-refresh-submit'] ) ) { // phpcs:ignore
			return;
		}
	 ...
	 * @since 7.0.0
	 */
	public function exactmetrics_notices() {
			...
			if ( ! empty( $addons_data ) ) {
				foreach ( $addons_data as $addons_type => $addons ) {
					foreach ( $addons as $addon ) {
						...
						if ( 'exactmetrics-ecommerce' === $slug && 'm' === $slug[0] ) {
							$addon = ExactMetrics()->routes->get_addon( $installed_plugins, $addons_type, $addon, $slug );
				...
				if ( ! empty( $plugin_info ) ) {
					foreach ( $plugin_info as $slug => $info ) {
						if ( strpos( $slug, 'exactmetrics-' ) !== false ) {
...
<div class="notice notice-warning is-dismissible">
	<p>
		<?php echo sprintf( esc_html__( 'Cannot activate %1$s%2$s%3$s. ExactMetrics PRO addon works with a valid and active licenses only.', 'exactmetrics' ), '<strong>', $addon_data->title, '</strong>' ); // phpcs:ignore ?>
    </p>
</div>
<?php
							}
