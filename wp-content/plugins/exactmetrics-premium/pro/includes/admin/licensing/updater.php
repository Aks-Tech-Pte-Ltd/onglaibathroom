...
class ExactMetrics_Updater {
	...
	public function __construct( array $config ) {
		...
		if ( strpos( site_url(), 'exactmetrics.com' ) !== false ) {
			return;
		}
	...
	public function maybe_show_license_expired_message( $plugin_data, $response ) {
		...
		if ( empty( $response->package ) ) {
			...
			// Translators: First one is a link to the settings page, second one is closing tag, third is a link to ExactMetrics.com
			echo '<br />' . sprintf( __( 'In order to enable updates, you need to have a valid license key on the %1$ssettings page%2$s. If your license key is expired or you need a new key, then %3$sclick here to purchase ExactMetrics Pro%2$s.', 'google-analytics-dashboard-for-wp' ), '<a href="' . esc_url( $settings_url ) . '">', '</a>', '<a href="' . exactmetrics_get_url( 'Plugin update', $this->plugin_name ) . '">' ); // phpcs:ignore
		}
	}
	...
	public function update_plugins_filter( $value ) {
		...
		if ( isset( $this->update->new_version ) && version_compare( $this->version, $this->update->new_version, '<' ) ) {
			...
			$this->update->exactmetrics_plugin  = true;
	...
	public function premium_update_push( $premium_update ) {
		...
		if ( ! empty( $update ) && version_compare( EXACTMETRICS_VERSION, $update->version, '<' ) ) {
