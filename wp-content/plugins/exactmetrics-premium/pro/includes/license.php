...
final class ExactMetrics_License {
	...
	public function get_site_license( $force = false ) {
		if ( ! empty( $this->site ) && ! $force ) {
			...
			$site       = get_option( 'exactmetrics_license', array() );
			$this->site = $site;
	...
	public function get_network_license( $force = false ) {
		if ( ! empty( $this->network ) && ! $force ) {
			...
			$network       = get_site_option( 'exactmetrics_network_license', array() );
	...
	public function get_license_key() {
		$license_key = ExactMetrics()->license->get_site_license_key();
		$license_key = ! empty( $license_key ) ? $license_key : ExactMetrics()->license->get_network_license_key();
		$license_key = ! empty( $license_key ) ? $license_key : ExactMetrics()->license->get_default_license_key();
	...
	public function get_site_license_updates() {
		return get_option( 'exactmetrics_license_updates', '' );
	}

	public function get_network_license_updates() {
		return get_site_option( 'exactmetrics_network_license_updates', '' );
	}

	public function set_site_license_updates() {
		update_option( 'exactmetrics_license_updates', strtotime( '+8 hours' ) );
	}

	public function set_network_license_updates() {
		update_site_option( 'exactmetrics_network_license_updates', strtotime( '+8 hours' ) );
	}

	public function delete_site_license_updates() {
		delete_option( 'exactmetrics_license_updates' );
	}

	public function delete_network_license_updates() {
		delete_site_option( 'exactmetrics_license_updates' );
	}

	public function time_to_check_site_license() {
		$timestamp = get_option( 'exactmetrics_license_updates' );
		if ( ! $timestamp ) {
			return true;
		} else {
	...
	public function time_to_check_network_license() {
		$timestamp = get_site_option( 'exactmetrics_network_license_updates' );
		if ( ! $timestamp ) {
			return true;
		} else {
			$current_timestamp = time();
	...
	public function get_site_license_error() {
		if ( $this->site_license_expired() ) {
			...
			return sprintf( esc_html__( 'Your license key for ExactMetrics has expired. %1$sPlease click here to renew your license key.%2$s', 'google-analytics-dashboard-for-wp' ), '<a href="' . exactmetrics_get_url( 'license-error', 'expired-license', 'https://www.exactmetrics.com/login/' ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
		} else if ( $this->site_license_disabled() ) {
			return esc_html__( 'Your license key for ExactMetrics has been disabled. Please use a different key.', 'google-analytics-dashboard-for-wp' );
		} else if ( $this->site_license_invalid() ) {
			return esc_html__( 'Your license key for ExactMetrics is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'google-analytics-dashboard-for-wp' );
		}
	...
	public function license_can( $requires = 'lite' ) {
		if ( ! exactmetrics_is_pro_version() || ! $this->has_license() ) {
	...
	public function get_default_license_key() {
		if ( defined( 'EXACTMETRICS_LICENSE_KEY' ) && is_string( EXACTMETRICS_LICENSE_KEY ) && strlen( EXACTMETRICS_LICENSE_KEY ) > 10 ) {
			return EXACTMETRICS_LICENSE_KEY;
		}
	...
	public function get_site_license_expiry_date() {
		if ( $this->site_license_expired() && ! empty( $this->site['expiry_date'] ) ) {

			if ( is_numeric( $this->site['expiry_date'] ) ) {
				$format = get_option( 'date_format' ) ? get_option( 'date_format' ) : 'F d, Y';

				return wp_date( $format, $this->site['expiry_date'] );
			}

			return $this->site['expiry_date'];
		};
	...
	public function get_network_license_expiry_date() {
		if ( $this->network_license_expired() && ! empty( $this->network['expiry_date'] ) ) {

			if ( is_numeric( $this->network['expiry_date'] ) ) {
				$format = get_option( 'date_format' ) ? get_option( 'date_format' ) : 'F d, Y';

				return wp_date( $format, $this->network['expiry_date'] );
			}
	...
	public function get_license_expiry_date() {
		if ( $this->is_network_licensed() ) {
			return $this->get_network_license_expiry_date();
		}

		return $this->get_site_license_expiry_date();
	}
}
