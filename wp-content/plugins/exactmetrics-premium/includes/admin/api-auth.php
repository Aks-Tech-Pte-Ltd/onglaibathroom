...
final class ExactMetrics_API_Auth {
	...
	public function __construct() {
        ...
        add_action( 'exactmetrics_v4_property_swap', array( $this, 'property_swap_cron_action' ) );
	}
	...
	public function delete_auth( $force = false ) {
		...
		if ( is_wp_error( $ret ) && ! $force ) {
			...
			if ( $this->is_network_admin() ) {
				ExactMetrics()->auth->delete_network_analytics_profile( false );
			} else {
				ExactMetrics()->auth->delete_analytics_profile( false );
