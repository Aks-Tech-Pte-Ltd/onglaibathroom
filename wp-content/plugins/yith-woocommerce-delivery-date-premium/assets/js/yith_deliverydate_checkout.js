jQuery(function ($) {
    ...
    var current_shipping_method_id = '',
        ...
        update_datepicker_form = function () {
            ...
            if (current_shipping_method_id !== shipping_id || typeof processing_method_id == 'undefined') {
                $.ajax({
                    type: 'POST',
                    url: ywcdd_params.ajax_url,
                    data: data,
        ...
        datepicker.datepicker({
            ...
            onClose: function (dateSelected, obj) {
                ...
                if (typeof availableDates !== 'undefined' && availableDates.indexOf(dateSelected) === -1) {
                    ...
                    if (dateSelected == '') {
                        alert(ywcdd_params.msgInvalidDate);
                    } else {
                        var message = ywcdd_params.msgUnavailableDate.replace("%s", dateSelected);
                        alert(message);
                    }
    ...
    $('form.checkout').on('change', '#ywcdd_timeslot', function (e) {
        ...
        if ('yes' === ywcdd_params.trigger_update_checkout) {
