...
/**
 * Manage the Product Quantity table post type list table
 *
 * @package YITH WooCommerce Delivery Date\Classes\Admin Tables
 */
...
if ( ! class_exists( 'YITH_Quantity_Table_List_Table' ) ) {
	/**
	 * Class YITH_Quantity_Table_List_Table
	 */
	class YITH_Quantity_Table_List_Table extends WP_List_Table {
		/**
		 * YITH_Quantity_Table_List_Table constructor.
		 *
		 ...
		 * Show the cb column
		 *
		 * @param int $item The post id.
		 *
		 * @return string
		 */
		public function column_cb( $item ) {
			return sprintf(
				'<input type="checkbox" name="ywcdd_quantity_table_ids[]" value="%s" />',
				$item
			);
		}
		...
		public function column_default( $item, $column_name ) {
			...
			switch ( $column_name ) {
				...
				case 'post_title':
					...
					$delete = ( isset( $_GET['status'] ) && 'trash' === $_GET['status'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					...
					if ( $delete ) {
						...
						$url_delete        = get_delete_post_link(
							$item,
							'',
							true
						);
						$actions['delete'] = '<a href="' . esc_url( $url_delete ) . '" class="submitdelete">' . __( 'Delete permanently', 'yith-woocommerce-delivery-date' ) . '</a>';
					} else {
						...
						$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=yith_product_table&action=duplicate_quantity_table&amp;post=' . $item ), 'woocommerce-duplicate-quantity-table_' . $item ) . '" aria-label="' . esc_attr__( 'Make a duplicate from this quantity table', 'yith-woocommerce-delivery-date' ) . '" rel="permalink">' . esc_html__( 'Duplicate', 'yith-woocommerce-delivery-date' ) . '</a>';
					}
				...
				case 'post_author':
					...
					echo sprintf( '<a href="%s" target="_blank">%s</a>', $user_link, $user->user_nicename ); //phpcs:ignore WordPress.Security.EscapeOutput
					break;

				case 'post_date':
					$post = get_post( $item );
					echo '<p>' . esc_html( $post->post_modified ) . '</p>';
					break;
				...
				case 'status':
					...
					echo $status; //phpcs:ignore WordPress.Security.EscapeOutput
					break;
				...
				case 'assign_at':
					...
					echo implode( ', ', $list_name );//phpcs:ignore WordPress.Security.EscapeOutput
					break;
				...
				case 'carrier':
					...
					if ( $carrier_id ) {
						echo esc_html( get_the_title( $carrier_id ) );
					}
					break;
			}
		...
		public function prepare_items() {
			...
			if ( isset( $_GET['status'] ) && 'all' !== $_GET['status'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

				$query_args['post_status'] = wp_unslash( $_GET['status'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			}

			if ( isset( $_GET['author'] ) && 'mine' === $_GET['author'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$query_args['author'] = wp_unslash( $_GET['author'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			}
		...
		/**
		 * Show the rows or the placeholder
		 *
		 * @since 1.0.0
		 */
		...
		public function get_bulk_actions() {
			...
			if ( isset( $_REQUEST['ywcdd_quantity_table_ids'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

				$rules = wp_unslash( $_REQUEST['ywcdd_quantity_table_ids'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

				if ( 'delete' === $actions || 'trash' === $actions ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
