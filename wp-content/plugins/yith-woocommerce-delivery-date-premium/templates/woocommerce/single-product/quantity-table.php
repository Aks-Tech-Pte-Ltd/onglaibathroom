...
<div id="ywcdd_quantity_table_container">
	...
	if ( $quantity_table_id ) {
		...
		$first_delivery_date   = YITH_Delivery_Date_Manager()->get_first_delivery_date(
			$carrier_id,
			array(
				'shipping_date' => $first_processing_date,
			)
		);
		...
		<style>
			...
			#ywcdd_quantity_table_wrap #ywcdd_quantity_table tbody td:hover, #ywcdd_quantity_table_wrap #ywcdd_quantity_table tbody td.ywcdd_day_selected {
				background-color: <?php echo get_option( 'ywcdd_table_customization_selected_quantity_color', '#c8b4c4' ); //phpcs:ignore WordPress.Security.EscapeOutput ?>;
			}
			...
			#ywcdd_quantity_table_wrap #ywcdd_quantity_table thead th.ywcdd_day:hover, #ywcdd_quantity_table_wrap #ywcdd_quantity_table thead th.ywcdd_day_selected {
				background-color: <?php echo get_option( 'ywcdd_table_customization_selected_day_color', '#a46497' ); //phpcs:ignore WordPress.Security.EscapeOutput ?>;
				border: 4px solid<?php echo get_option( 'ywcdd_table_customization_selected_day_color', '#a46497' ); //phpcs:ignore WordPress.Security.EscapeOutput ?>;
			}
			...
			<table id="ywcdd_quantity_table">
				...
				<tr>
					...
					for ( $i = 0; $i < 4; $i ++ ) {
						...
						$date = new WC_DateTime( date( 'Y-m-d', $delivery_dates[ $i ] ), new DateTimeZone( 'UTC' ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions

						$last_shipping_date = YITH_Delivery_Date_Manager()->get_last_shipping_date(
							$delivery_dates[ $i ],
							$processing_method,
							$carrier_id,
							array(
						...
						?>
						<th class="ywcdd_day day_<?php echo esc_attr( ( $i + 1 ) ); ?> <?php echo esc_attr( $class_disabled ); ?>" data-day_column="<?php echo esc_attr( ( $i + 1 ) ); ?> ">
							<div data-delivery_date="<?php echo esc_attr( date( 'Y-m-d', $delivery_dates[ $i ] ) ); ?>" data-last_shipping_date="<?php echo esc_attr( $last_shipping_date ); ?>"><?php echo( $formatted_date ); //phpcs:ignore WordPress.Security.EscapeOutput,WordPress.DateTime.RestrictedFunctions ?></div>
						</th>
						<?php
					}
					?>
				...
				<tr>
					<th class="ywcdd_quantity"><?php esc_html_e( 'Quantity', 'yith-woocommerce-delivery-date' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<?php
				foreach ( $quantity_price as $index => $row ) :
					...
					?>
					<tr>
						<td class="ywcdd_quantity" data-qty="<?php echo esc_attr( $quantity ); ?>">
							<span><?php echo esc_html( $quantity ); ?></span>
						</td>
						<?php
						foreach ( $row_days as $n_day => $day ) :
							...
							if ( 'yes' === $day['enabled'] && $enough_in_stock ) {
								...
								if ( $new_max_price && $new_price < $new_max_price ) {
									...
									$original_min_price_disp = wc_get_price_to_display(
										$product,
										array(
									...
									$original_max_price_disp = wc_get_price_to_display(
										$product,
										array(
									...
									$new_price_dp     = wc_get_price_to_display(
										$product,
										array(
											'price' => $new_price,
											'qty'   => $quantity,
									...
									$new_max_price_dp = wc_get_price_to_display(
										$product,
										array(
											'price' => $new_max_price,
											'qty'   => $quantity,
									...
									$price_from = wc_get_price_to_display(
										$product,
										array(
											'price' => $original_min_price,
											'qty'   => $quantity,
									...
									$price_to = wc_get_price_to_display(
										$product,
										array(
											'price' => $new_price,
											'qty'   => $quantity,
							...
							?>
							<td class="ywcdd_day day_<?php echo esc_attr( ( $n_day + 1 ) ); ?> <?php echo esc_attr( $col_disabled ); ?>"
								data-price="<?php echo esc_attr( $new_price ); ?>" data-day_column="<?php echo esc_attr( ( $n_day + 1 ) ); ?>">
								<span>
									<?php echo $price_formatted; //phpcs:ignore WordPress.Security.EscapeOutput ?>
								</span>
							</td>
						<?php endforeach; ?>
					</tr>
						...
						<table>
							<tr>
								<td id="ywcdd_delivery_recap_date">
									<p><?php esc_html_e( 'Delivery', 'yith-woocommerce-delivery-date' ); ?></p>
									<p class="ywcdd_recap_value"></p>
								</td>
								<td id="ywcdd_delivery_recap_quantity">
									<p><?php esc_html_e( 'Quantity', 'yith-woocommerce-delivery-date' ); ?></p>
									<p class="ywcdd_recap_value"></p>
								</td>
								<td id="ywcdd_delivery_recap_total">
									<p><?php esc_html_e( 'Total price', 'yith-woocommerce-delivery-date' ); ?></p>
									<p class="ywcdd_recap_value"></p>
								</td>
							</tr>
						</table>
		...
		<input type="hidden" name="ywcdd_processing_method_id" value="<?php echo esc_attr( $processing_method ); ?>">
		<input type="hidden" name="ywcdd_carrier_id" value="<?php echo esc_attr( $carrier_id ); ?>">
		<?php
