<?php
/**
 * Custom field, to show a select for the available shipping method
 *
...
foreach ( $shipping_zones as $zone ) {
	...
	foreach ( $shipping_methods as $method ) {
		...
		if ( isset( $method->instance_settings['select_process_method'] ) && intval( $post->ID ) === intval( $method->instance_settings['select_process_method'] ) ) {
...
<div id="<?php echo esc_attr( $id ); ?>-container" <?php echo $deps_html; //phpcs:ignore WordPress.Security.EscapeOutput ?>>
	<div id="ywcdd_shipping_table">
		<label><?php esc_html_e( 'Shipping Method', 'yith-woocommerce-delivery-date' ); ?></label>
		<table class="widefat wc-shipping-zone-methods">
			<thead>
			<tr>
				<th><?php esc_html_e( 'Shipping Zone', 'yith-woocommerce-delivery-date' ); ?></th>
				<th><?php esc_html_e( 'Shipping Method', 'yith-woocommerce-delivery-date' ); ?></th>
				<th><?php esc_html_e( 'Required', 'yith-woocommerce-delivery-date' ); ?></th>
			</tr>
			</thead>
			...
			if ( count( $shipping_method_av ) > 0 ) {
				foreach ( $shipping_method_av as $key => $method_available ) {
					...
					foreach ( $shipping_methods as $shipping_method ) {
						...
						echo '<td>' . esc_html( $shipping_method['name'] ) . '</td>';
						echo '<td>' . esc_html( $shipping_method['required'] ) . '</td>';
						echo '</tr>';
					}
					?>
			...
			<?php } else { ?>
				<tr>
					<td colspan="3" class="ywcdd_no_shipping_method"><?php esc_html_e( 'This method is not set in any WooCommerce Shipping Method', 'yith-woocommerce-delivery-date' ); ?></td>
			...
			<tr>
				<td colspan="3">
					...
					echo sprintf( '<a href="%s" target="_blank" class="button button-primary button-small">%s</a>', $zone_url, __( 'Manage Shipping Zones', 'yith-woocommerce-delivery-date' ) ); //phpcs:ignore WordPress.Security.EscapeOutput
					?>
				</td>
			</tr>
			</tfoot>
