<?php
/**
 * Custom field, to show the quantity table in product table feature
 *
...
<div id="<?php echo esc_attr( $id ); ?>-container" class="yith-plugin-fw-metabox-field-row">
				...
				<table>
					...
					<tr>
						...
						for ( $i = 0; $i < $column; $i ++ ) {
							...
							?>
							<th><?php echo sprintf( $header_title, $i + 1 ); //phpcs:ignore WordPress.Security.EscapeOutput ?></th>
							<?php
						}
						?>
					...
					<?php foreach ( $value as $i => $table ) : ?>
						<tr>
							<td class="ywcdd_quantity">
								...
								echo yith_plugin_fw_get_field( $qyt_args, true ); //phpcs:ignore WordPress.Security.EscapeOutput
								?>
							...
							foreach ( $days as $k => $day ) {
								...
								?>
								<td>
									<?php echo yith_plugin_fw_get_field( $onoff_field, true ); //phpcs:ignore WordPress.Security.EscapeOutput ?>
									<div class="yith-plugin-fw-field-wrapper ywcdd_product_quantity_value_wrapper">
										<?php echo yith_plugin_fw_get_field( $value_type_field, true ); //phpcs:ignore WordPress.Security.EscapeOutput ?>
										<?php echo yith_plugin_fw_get_field( $value_field, true ); //phpcs:ignore WordPress.Security.EscapeOutput ?>
									</div>
								</td>
								<?php
							}
							...
							<td class="ywcdd_remove_row_column">
								<?php if ( $i > 0 ) : ?>
								<a href="" class="ywcdd_remove_row" title=""><span class="yith-icon-trash"></span></a>
								<?php endif; ?>
							</td>
					...
					<tr>
						...
						<td colspan="4">
							<div class="ywcdd_add_new_row">
								<button class="button-primary yith-add-button"><?php esc_html_e( 'Add row', 'yith-woocommerce-delivery-date' ); ?></button>
							</div>
						</td>
					</tr>
					</tfoot>
	...
	<script type="text/template" id="tmpl-ywcdd-product-table-row">
		<tr>
			...
			?>
			<td class="ywcdd_remove_row_column"><a href="" class="ywcdd_remove_row" title=""><span class="yith-icon-trash"></span></a></td>
		</tr>
	</script>
</div>
