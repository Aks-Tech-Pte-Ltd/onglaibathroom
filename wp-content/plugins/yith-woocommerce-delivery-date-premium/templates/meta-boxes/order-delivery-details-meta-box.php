<?php
/**
 * This file show the delivery info in the order with a meta box
 *
 * @package YITH WooCommerce Delivery Date\Templates\Meta Boxes
 */
...
/**
 * APPLY_FILTERS: ywcdd_order_shipped_default_value
 *
 * Check if show the default value or not.
 *
 * @param string $show_default Yes or no.
 *
 * @return string
 */
$default_shipped    = apply_filters( 'ywcdd_order_shipped_default_value', 'no' );
$order_shipped      = empty( $order_shipped ) ? $default_shipped : $order_shipped;
...
if ( ( 'product' === $option && ! empty( $shipping_date ) ) || 'checkout' === $option ) {
    ...
    <div id="ywcdd_delivery_order_metabox">
        <h4>
			<?php esc_html_e( 'Processing and Delivery date', 'yith-woocommerce-delivery-date' ); ?>
			<?php
			/**
			 * APPLY_FILTERS: ywcdd_edit_order_date_details
			 *
			 * Check if show the edit metabox.
			 *
			 * @param bool $show_edit True or false.
			 * @param WC_Order $order The order.
			 *
			 * @return bool
			 */
			if ( apply_filters( 'ywcdd_edit_order_date_details', ! in_array( $order->get_status(), array(
				'completed',
				'refunded'
			), true ), $order ) ) {
				?>
                <a href="#" class="edit_address"><?php esc_html_e( 'Edit', 'yith-woocommerce-delivery-date' ); ?></a>
				<?php
			}
			?>
        </h4>
        <div id="processing_delivery_date_container">
            <div class="processing_delivery_date">
				...
				foreach ( $fields as $key => $field ) {
					...
					if ( 'shipping_date' === $key ) {
						...
						if ( $formatted_value ) {
							...
							echo sprintf( '<p class="%s"><strong>%s:</strong>%s<br/>%s</p>', $key, $field['label'], $shipping_message, $formatted_value );//phpcs:ignore WordPress.Security.EscapeOutput
						} else {
							echo sprintf( '<p class="%s"><strong>%s:</strong>%s<br/></p>', $key, $field['label'], $no_available_label );//phpcs:ignore WordPress.Security.EscapeOutput
						}
					} else {

						if ( $formatted_value ) {
							echo sprintf( '<p class="%s"><strong>%s:</strong><br/>%s</p>', $key, $field['label'], $formatted_value );//phpcs:ignore WordPress.Security.EscapeOutput
						} else {
							echo sprintf( '<p class="%s"><strong>%s:</strong><br/>%s</p>', $key, $field['label'], $no_available_label );//phpcs:ignore WordPress.Security.EscapeOutput
						}
					}
				}
				?>
                <p>
                    <label
                            for="ywcdd_order_shipped"><strong><?php esc_html_e( 'Shipped to carrier', 'yith-woocommerce-delivery-date' ); ?></strong></label>
                    <input type="checkbox" id="ywcdd_order_shipped" name="ywcdd_order_shipped"
                           value="1" <?php checked( 'yes', $order_shipped ); ?> <?php echo esc_attr( $disable_option ); ?>/>
					<?php echo $disable_tooltip; //phpcs:ignore WordPress.Security.EscapeOutput ?>
                </p>
            </div>
            <div class="edit_processing_delivery_date">
                <p class="edit_processing_method">
                    <label for="ywcdd_edit_processing_method"><?php esc_html_e( 'Processing Method', 'yith-woocommerce-delivery-date' ); ?></label>
					<?php
					$all_processing_method = YITH_Delivery_Date_Processing_Method()->get_processing_method( array( 'fields' => 'ids' ) );
					?>
                    <select id="ywcdd_edit_processing_method" name="ywcdd_edit_processing_method">
                        <option value="" <?php selected( '', $method_id ); ?>><?php esc_html_e( 'Select a Processing Method', 'yith-woocommerce-delivery-date' ); ?></option>
						<?php foreach ( $all_processing_method as $processing_method_id ) : ?>
                            <option value="<?php echo esc_attr( $processing_method_id ); ?>" <?php selected( $processing_method_id, $method_id ); ?>><?php echo esc_html( get_the_title( $processing_method_id ) ); ?></option>
						<?php endforeach; ?>
                    </select>
                </p>
                <p class="edit_carrier">
                    <label for="ywcdd_edit_carrier">
						<?php esc_html_e( 'Carrier', 'yith-woocommerce-delivery-date' ); ?>
                    </label>
					<?php
					$all_carrier = YITH_Delivery_Date_Carrier()->get_all_formatted_carriers();
					?>
                    <select id="ywcdd_edit_carrier" name="ywcdd_edit_carrier">
                        <option value="" <?php selected( '', $carrier_id ); ?>><?php esc_html_e( 'Select a Carrier', 'yith-woocommerce-delivery-date' ); ?></option>
						<?php foreach ( $all_carrier as $id => $carrier_name ) : // phpcs:ignore WordPress.WP.GlobalVariablesOverride ?>
                            <option value="<?php echo esc_attr( $id ); ?>" <?php selected( $carrier_id, $id ); ?>><?php echo esc_html( $carrier_name ); ?></option>
						<?php endforeach; ?>
                    </select>
                </p>
                <p class="edit_processing_date">
                    <label for="ywcdd_edit_processing_date">
						<?php esc_html_e( 'Processing date', 'yith-woocommerce-delivery-date' ); ?>
                    </label>
                    <input id="ywcdd_edit_processing_date" type="text" class="ywcdd_datepicker"
                           value="<?php echo esc_attr( $shipping_date ); ?>" name="ywcdd_edit_processing_date">
                </p>
                <p class="edit_delivery_date">
                    <label for="ywcdd_edit_delivery_date">
						<?php esc_html_e( 'Delivery date', 'yith-woocommerce-delivery-date' ); ?>
                    </label>
                    <input id="ywcdd_edit_delivery_date" type="text" class="ywcdd_datepicker"
                           value="<?php echo esc_attr( $delivery_date ); ?>" name="ywcdd_edit_delivery_date">
                </p>
                <p class="edit_time_from">
                    <label for="ywcdd_edit_time_from">
						<?php esc_html_e( 'Time From', 'yith-woocommerce-delivery-date' ); ?>
                    </label>
                    <input id="ywcdd_edit_time_from" type="text" class="ywcdd_timepicker"
                           value="<?php echo esc_attr( $time_from ); ?>" name="ywcdd_edit_time_from">
                </p>
                <p class="edit_time_to">
                    <label for="ywcdd_edit_time_to">
						<?php esc_html_e( 'Time To', 'yith-woocommerce-delivery-date' ); ?>
                    </label>
                    <input id="ywcdd_edit_time_to" type="text" class="ywcdd_timepicker"
                           value="<?php echo esc_attr( $time_to ); ?>" name="ywcdd_edit_time_to">
                </p>
                <button class="button-secondary ywcdd_update_date"><?php esc_html_e( 'Update delivery details', 'yith-woocommerce-delivery-date' ); ?></button>
            </div>
        </div>
    </div>
	<?php
